var Whizzie = (function($){
    ...
    var callbacks = {
		...
		install_widgets: function( btn ) {
            free_demo_content_type="customize";
			var widgets = new WidgetManager(free_demo_content_type);
			widgets.init( btn );
		},
    ...
    function window_loaded() {
		...
		$( '.whizzie-menu li.step:first-child' ).addClass( 'nav-tab-active' );
		$( '.whizzie-nav li:first-child' ).addClass( 'nav-tab-active' );

		$( '.whizzie-wrap' ).addClass( 'loaded test' );
    ...
    function do_next_step( btn ) {
		...
		current_step.removeClass( 'nav-tab-active' );
		$( '.nav-step-' + step_pointer ).removeClass( 'nav-tab-active' );
		...
		current_step.fadeOut( 500, function() {
			...
			current_step.fadeIn();
			current_step.addClass( 'nav-tab-active' );
			$( '.nav-step-' + step_pointer ).addClass( 'nav-tab-active' );
    ...
    function PluginManager(){
                ...
                if(typeof response.url != 'undefined'){
                    ...
                    if(response.hash == current_item_hash){
                        ...
                        jQuery.post(response.url, response, function(response2) {
                            process_current();
                            $current_node.find('.wizard-plugin-status').text(response.message + pixel_ebook_store_whizzie_params.verify_text);
                        }).fail(ajax_callback);
        ...
        function process_current() {
            if(current_item){
                ...
                jQuery.post(pixel_ebook_store_whizzie_params.ajaxurl, {
                    action: 'setup_plugins',
                    wpnonce: pixel_ebook_store_whizzie_params.wpnonce,
                    slug: current_item
        ...
        function find_next(){
            ...
            if(items_completed >= $li.length){
                ...
                $('.step.step-plugins .button').text('Go To Next Step');
        ...
        return {
            ...
            init: function(btn){
                if(jQuery('.step.step-plugins .button').text() != "Go To Next Step"){
	...
	function WidgetManager(demo_type) {
        ...
        if(demo_type == 'builder'){
    		...
    		function import_widgets(){
                jQuery.post(
    				pixel_ebook_store_whizzie_params.ajaxurl,
    				{
    					wpnonce: pixel_ebook_store_whizzie_params.wpnonce
    				}, ajax_callback).fail(ajax_callback);
    		}
...
jQuery(document).ready(function(){
    ...
    if(tabenable==1){
        ...
        jQuery('.wizard-menu-page li.step').removeClass('nav-tab-active');
        jQuery('.wizard-menu-page li.step').css('display','none');
        jQuery('.wizard-icon-nav li').removeClass('nav-tab-active');
        jQuery('.wizard-menu-page .' + current_icon_step).addClass('nav-tab-active');
    ...
    if(plugin_count == 0){
        ...
        jQuery('.step.step-plugins a.button').text('Go To Next Step');
